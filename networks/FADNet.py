...
from networks.DispNetC import ExtractNet, CUNet, NAS_CUNet
from networks.DispNetRes import DispNetRes
from networks.submodules import build_corr, warp_right_to_left, channel_length
import copy
...
class FADNet(nn.Module):
    ...
    def __init__(self, resBlock=True, maxdisp=192, input_channel=3, encoder_ratio=16, decoder_ratio=16,
                 feature_blocks=None, ks_list=7, expand_ratio_list=8, depth_list=4, num_scales=3):
        ...
        # First Block (Extract)
        if self.is_NAS:
            self.extract_network = nn.ModuleList(feature_blocks)
            self.cunet = NAS_CUNet(maxdisp=self.maxdisp,
            ...
            self.extract_network = ExtractNet(resBlock=resBlock, maxdisp=self.maxdisp, input_channel=input_channel,
                                              encoder_ratio=encoder_ratio, decoder_ratio=decoder_ratio)
            self.cunet = CUNet(resBlock=resBlock, maxdisp=self.maxdisp,
                               input_channel=input_channel,
                               encoder_ratio=encoder_ratio, decoder_ratio=decoder_ratio)
        # Second Block (Encoder-Decoder)

        # Third Block (DispNetRes), input is 11 channels(img0, img1, img1->img0, flow, diff-mag)
        in_planes = 3 * 3 + 1 + 1
        self.dispnetres = DispNetRes(in_planes, resBlock=resBlock, input_channel=input_channel,
                                     encoder_ratio=encoder_ratio, decoder_ratio=decoder_ratio)
    ...
    def forward(self, inputs, enabled_tensorrt=False):
        ...
        # remove to ofa_fadnet.feature_extraction

        # extract features

        # conv1_l, conv2_l, conv3a_l, conv3a_r = self.extract_network(inputs)

        conv1_l, conv2_l, conv3a_l, conv3a_r = self.feature_extraction(inputs)
        # build corr
        out_corr = build_corr(conv3a_l, conv3a_r, max_disp=self.maxdisp // 8 + 16)
        # generate first-stage flows

        dispnetc_flows = self.cunet(inputs, conv1_l, conv2_l, conv3a_l, out_corr)
        # lambda input: self.cunet_nas_feature_extraction(input))
        # dispnetc_flows = self.cunet(inputs, conv1_l, conv2_l, conv3a_l, out_corr)
