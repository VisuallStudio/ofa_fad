...
def profile(model, input_size, custom_ops=None):
    ...
    def add_hooks(m_):
        ...
        m_.register_buffer('total_ops', torch.zeros(1,device='cuda:0'))
        m_.register_buffer('total_params', torch.zeros(1,device='cuda:0'))
        # print(m_.total_ops.device) cuda:0
        for p in m_.parameters():
            m_.total_params += torch.zeros(1,device='cuda:0').fill_(p.numel())
    ...
    with torch.no_grad():
        model(x)
    # print(next(model.parameters()).device) cuda0
    total_ops =0
    total_params =0
    for m in model.modules():
        if len(list(m.children())) > 0:  # skip for non-leaf module
            continue
        m.total_ops=m.total_ops.to('cuda:0')
