...
from utils.common import min_divisible_value, make_divisible, Hsigmoid
...
class MyConvTranspose2d(nn.ConvTranspose2d):
    """
    Conv2d with Weight Standardization
    https://github.com/joe-siyuan-qiao/WeightStandardization
    """
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
        ...
        super(MyConvTranspose2d, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias)
        self.WS_EPS = None
    ...
    def forward(self, input,output_size):
        ...
        if self.WS_EPS is None:
            return super(MyConvTranspose2d, self).forward(x)
        else:
            return F.conv_transpose2d(x, self.weight_standardization(self.weight), self.bias,
                            self.stride, self.padding, self.dilation, self.groups)

    def __repr__(self):
        return super(MyConvTranspose2d, self).__repr__()[:-1] + ', ws_eps=%s)' % self.WS_EPS
