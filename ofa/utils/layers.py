...
from ofa.utils.my_modules import SEModule, ShuffleLayer
from ofa.utils.my_modules import MyNetwork, MyModule
from utils.common import build_activation, make_divisible, get_same_padding, min_divisible_value
...
def set_layer_from_config(layer_config):
    ...
    name2layer = {
        ...
        ConvTransposeLayer.__name__:ConvTransposeLayer,
...
class ConvTransposeLayer(My2DLayer):

    def __init__(self, in_channels, out_channels,
                 kernel_size=3, stride=1, dilation=1, groups=1, bias=False, has_shuffle=False, use_se=False,
                 use_bn=True, act_func='LeakyReLU', dropout_rate=0, ops_order='weight_bn_act'):
        # default normal 3x3_Conv with bn and relu
        self.kernel_size = kernel_size
        self.stride = stride
        self.dilation = dilation
        ...
        super(ConvTransposeLayer, self).__init__(in_channels, out_channels, use_bn, act_func, dropout_rate, ops_order)
        if self.use_se:
            self.add_module('se', SEModule(self.out_channels))
    ...
    def module_str(self):
        ...
        if self.groups == 1:
            if self.dilation > 1:
                deconv_str = '%dx%d_DilatedConvTranspose' % (kernel_size[0], kernel_size[1])
            else:
                deconv_str = '%dx%d_ConvTranspose' % (kernel_size[0], kernel_size[1])
        else:
            if self.dilation > 1:
                deconv_str = '%dx%d_DilatedGroupConvTranspose' % (kernel_size[0], kernel_size[1])
            else:
                deconv_str = '%dx%d_GroupConvTranspose' % (kernel_size[0], kernel_size[1])
        deconv_str += '_O%d' % self.out_channels
        if self.use_se:
            deconv_str = 'SE_' + deconv_str
        deconv_str += '_' + self.act_func.upper()
        if self.use_bn:
            if isinstance(self.bn, nn.GroupNorm):
                deconv_str += '_GN%d' % self.bn.num_groups
            elif isinstance(self.bn, nn.BatchNorm2d):
                deconv_str += '_BN'
        return deconv_str
    ...
    def config(self):
        return {
            'name': ConvTransposeLayer.__name__,
            'kernel_size': self.kernel_size,
            'stride': self.stride,
            'dilation': self.dilation,
            'groups': self.groups,
            ...
            'has_shuffle': self.has_shuffle,
            'use_se': self.use_se,
            **super(ConvTransposeLayer, self).config
        }

    @staticmethod
    def build_from_config(config):
        return ConvTransposeLayer(**config)
