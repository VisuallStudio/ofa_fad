...
from utils.common import get_same_padding, sub_filter_start_end, make_divisible
from ofa.utils.my_modules import MyNetwork, MyConv2d, SEModule

__all__ = ['DynamicSeparableConv2d', 'DynamicConv2d', 'DynamicConvTranspose2d', 'DynamicGroupConv2d',
...
class DynamicConvTranspose2d(nn.Module):
    ...
    def __init__(self, max_in_channels, max_out_channels, kernel_size=1, stride=1, dilation=1, bias=False):
        ...
        self.deconv = nn.ConvTranspose2d(
            self.max_in_channels, self.max_out_channels, self.kernel_size, stride=self.stride, bias=bias,
        )

        self.active_out_channel = self.max_out_channels

    def get_active_filter(self, out_channel, in_channel):
        return self.deconv.weight[:out_channel, :in_channel, :, :]
    ...
    def forward(self, x, out_channel=None):
        ...
        padding = get_same_padding(self.kernel_size)
        filters = self.deconv.weight_standardization(filters) if isinstance(self.deconv, MyConvTranspose2d) else filters
        y = F.conv_transpose2d(x, filters, None, self.stride, padding, self.dilation)
        return y
...
class DynamicSE(SEModule):
    ...
    def forward(self, x, groups=None):
        ...
        reduce_filter=reduce_filter.to(x.device)
        reduce_bias=reduce_bias.to(x.device)
        ...
        expand_filter = expand_filter.to(y.device)
        expand_bias = expand_bias.to(y.device)
