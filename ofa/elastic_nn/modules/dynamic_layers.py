...
from ofa.utils.layers import ResNetBottleneckBlock, LinearLayer, ResidualBlock
from utils.common import val2list, get_net_device, build_activation, make_divisible
from ofa.utils.my_modules import MyModule, MyNetwork, SEModule
from .dynamic_op import DynamicSeparableConv2d, DynamicConv2d, DynamicConvTranspose2d, DynamicBatchNorm2d, DynamicSE, \
    DynamicGroupNorm
...
class DynamicConvTransposeLayer(MyModule):

    def __init__(self, in_channel_list, out_channel_list, kernel_size=3, stride=1, dilation=1,
                 act_func='LeakyRelu'):
        super(DynamicConvTransposeLayer, self).__init__()
        ...
        self.deconv = DynamicConvTranspose2d(
            max_in_channels=max(self.in_channel_list), max_out_channels=max(self.out_channel_list),
            kernel_size=self.kernel_size, stride=self.stride, dilation=self.dilation
        )
    ...
    def forward(self, x):
        self.deconv.active_out_channel = self.active_out_channel
    ...
    @property
    def module_str(self):
        return 'DyConvTranspose(O%d, K%d, S%d)' % (self.active_out_channel, self.kernel_size, self.stride)

    @property
    def config(self):
        return {
            'name': DynamicConvTransposeLayer.__name__,
            'in_channel_list': self.in_channel_list,
            'out_channel_list': self.out_channel_list,
            'kernel_size': self.kernel_size,
            'stride': self.stride,
    ...
    @staticmethod
    def build_from_config(config):
        return DynamicConvTransposeLayer(**config)
    ...
    def get_active_subnet(self, in_channel, preserve_weight=True):
        ...
        sub_layer.deconv.weight.data.copy_(self.deconv.get_active_filter(self.active_out_channel, in_channel).data)
...
class DynamicResBlock(MyModule):
    ...
    def __init__(self, in_channel_list, out_channel_list, expand_ratio_list=0.25,
        ...
        if stride != 1 or self.out_channel_list != self.in_channel_list:
    ...
    def forward(self, x):
        ...
        if not isinstance(self.shortcut, IdentityLayer):
            self.shortcut.conv.active_out_channel = self.active_out_channel
    ...
    def module_str(self):
        return '(%s, %s)' % (
            '%dx%d_resConv_in->%d->%d_S%d' % (
                self.kernel_size, self.kernel_size, self.active_middle_channels, self.active_out_channel, self.stride
            ),
            'shortcut' if isinstance(self.shortcut, IdentityLayer) else self.shortcut,
        )
