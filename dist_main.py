...
def main(opt):
    ...
    trainer = DisparityTrainer(opt.net, opt.lr, [0], opt.dataset, opt.trainlist, opt.vallist, opt.datapath,
                               opt.batch_size, opt.maxdisp,
                               opt.model, ngpu=ngpu, rank=rank, hvd=True, )
    ...
    if trainer.is_pretrain:
        # with torch.no_grad():
            # trainer.net.re_organize_middle_weights(expand_ratio_stage=2)
            # best_EPE = trainer.validate()
        print("跳过模型加载后验证的阶段先~")


    # train elastic expand ratio
    # trainer.net.re_organize_middle_weights(expand_ratio_stage=2)
    ...
    for r in range(opt.startRound, train_round):
        ...
        for i in range(start_epoch, end_epoch):
            ...
            if rank == 0:
                ...
                logger.info('Validation [round:%d,epoch:%d]: ' % (r, i) + '\t'.join(
                    [datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), str(avg_loss), str(float(avg_EPE)),
                     str(float(val_EPE)), str(trainer.current_lr)]))

                # wandb.log({'train-epe': float(avg_EPE), 'val-epe': float(val_EPE), 'best-val-epe': float(best_EPE), 'epoch': i+1, 'round': r+1, 'curr_lr': trainer.current_lr})
        start_epoch = 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--momentum', type=float, default=0.9,
                        help='momentum for sgd, alpha parameter for adam. default=0.9')
    ...
    parser.add_argument('--startRound', type=int, help='the round number to start training, useful of lr scheduler',
                        default='0')
    parser.add_argument('--startEpoch', type=int, help='the epoch number to start training, useful of lr scheduler',
                        default='0')
    ...
    parser.add_argument('--trainlist', type=str, help='provide the train file (with file list)',
                        default='FlyingThings3D_release_TRAIN.list')
    parser.add_argument('--vallist', type=str, help='provide the val file (with file list)',
                        default='FlyingThings3D_release_TEST.list')
    ...
    #     wandb.init(project='nas_fadnet', entity='ssttjka', name=logfile, config=opt)
