dnn="${dnn:-fadnet}"
nworkers="${nworkers:-4}"
datathread="${datathread:-4}"
source exp_configs/$dnn.conf
MPIPATH=/home/jingbo/software/openmpi-4.1.5
PY=/home/jingbo/anaconda3/envs/fadnet/bin/python
#lr=1e-3

rdma="${rdma:-0}"
if [ "$rdma" = "0" ]; then
params="-mca pml ob1 -mca btl ^openib \
    -mca btl_tcp_if_include 192.168.3.109/24 \
    -x NCCL_DEBUG=INFO  \
    -x NCCL_SOCKET_IFNAME=eno1 \
    -x NCCL_IB_DISABLE=1 \
    -x HOROVOD_CACHE_CAPACITY=0 \
    -x datathread=${datathread}"
else
