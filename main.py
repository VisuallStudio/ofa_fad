...
def main(opt):
    ...
    loss_weights = loss_json["loss_weights"]  # 4x7  多尺度loss权值训练策略
    ...
    trainer = DisparityTrainer(opt.net, opt.lr, opt.devices, opt.dataset, opt.trainlist, opt.vallist, opt.datapath,
                               opt.batch_size, opt.maxdisp, opt.model)
    ...
    for r in range(opt.startRound, train_round):
        ...
        for i in range(start_epoch, end_epoch):
            ...
            logger.info('Validation [round:%d,epoch:%d]: ' % (r, i) + '\t'.join(
                [datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), str(avg_loss), str(avg_EPE), str(val_EPE),
                 str(trainer.current_lr)]))
        start_epoch = 0
